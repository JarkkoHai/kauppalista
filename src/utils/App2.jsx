//<artifact identifier="fixed-app-jsx" type="application/vnd.ant.code" language="javascript" title="Korjattu App.jsx">
...
const [needsProCheck, setNeedsProCheck] = useState(false); // ← UUSI FLAG
const [session, setSession] = useState(() => {
const saved = localStorage.getItem('shopping_session_pro_v2');
return saved ? JSON.parse(saved) : null;
});
console.log('🟣 MainApp - session:', session, 'user:', user?.uid, 'needsProCheck:', needsProCheck);
// Auth state listener
useEffect(() => {
...
setUser(u);
    // Jos Pro+ käyttäjä kirjautuu ilman sessiota
    if (!session && !u.isAnonymous && !isLoggingOut) {
      ...
      if (existingList.success) {
        ...
        console.log('📂 No list found, creating one and checking Pro status');
        setNeedsProCheck(true); // ← Merkitse että tarvitaan Pro tarkistus
      }
    }
  }
});
...
// Pro status check - vain kun needsProCheck = true
useEffect(() => {
if (!needsProCheck || !user || user.isAnonymous) return;
const checkProStatus = async () => {
  console.log('🔵 Checking Pro+ status for user:', user.uid);
  ...
  const result = await joinList(newCode, user.uid);
  
  if (result.success) {
    const newSession = { 
      code: result.code, 
      isPro: isProUser // ← Käytä todellista Pro statusta
    };
    setSession(newSession);
    localStorage.setItem('shopping_session_pro_v2', JSON.stringify(newSession));
    
    // Jos EI ole Pro, näytä pricing
    if (!isProUser) {
      console.log('💳 User not Pro+, showing pricing...');
      setShowPricingModal(true);
    }
  }
  
  setNeedsProCheck(false); // ← Nollaa flag
};
...
onProLogin={(user) => {
console.log('🔵 Pro+ login callback, user:', user.uid);
setUser(user);
setNeedsProCheck(true); // ← Laukaise Pro tarkistus
}}
/>
);
}
